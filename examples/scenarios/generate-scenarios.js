...
/**
 * Generate all scenario pages from template
 */
...
// Read template
const templatePath = path.join(__dirname, 'scenario-template.html');
...
scenarios.forEach(scenario => {
  ...
  const outputPath = path.join(__dirname, `${scenario.id}.html`);
  fs.writeFileSync(outputPath, content);
  console.log(`Generated: ${scenario.id}.html`);
});

console.log(`\nGenerated ${scenarios.length} scenario pages.`);
